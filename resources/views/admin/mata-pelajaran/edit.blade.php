...
<div class="max-w-2xl mx-auto">
    <x-card>
        <form action="{{ route('admin.mata-pelajaran.update', $mataPelajaran) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="tingkat"
                        ...
                        required>
                    @for($i = 1; $i <= 6; $i++)
                    <option value="{{ $i }}" {{ old('tingkat', $mataPelajaran->tingkat) == $i ? 'selected' : '' }}>
                        Kelas {{ $i }}
                    </option>
                    @endfor
                </select>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                <button type="button"
                        onclick="if(confirm('Yakin ingin menghapus mata pelajaran ini?')) document.getElementById('delete-form').submit()"
        ...
        <form id="delete-form" action="{{ route('admin.mata-pelajaran.destroy', $mataPelajaran) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </x-card>
